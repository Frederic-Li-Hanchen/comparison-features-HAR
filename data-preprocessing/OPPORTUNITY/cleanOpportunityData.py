...
dataFolder = '/hdd/datasets/OPPORTUNITY/raw'

# Path to the folder to save the new clean data files
resultFolder = '/hdd/datasets/OPPORTUNITY/clean/rla_sensors'
...
def cleanOpportunityData(dataFolder, resultFolder, allSensors=True):
    ...
    if allSensors:
        sensorId = list(range(1,13)) + list(range(16,34)) + list(range(37,46)) + list(range(50,59)) + list(range(63,72)) + list(range(76,85)) + list(range(89,98)) + list(range(102,134)) + [249]
    else:
        sensorId = [63,64,65,66,67,68,249]
    ...
    for file in os.listdir(dataFolder):
        ...
        if file.endswith('.dat'):
            ...
            #pdb.set_trace()
            np.savetxt(resultFolder+'/'+outputFileName,data,delimiter=' ',fmt='%d')
