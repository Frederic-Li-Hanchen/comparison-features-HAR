...
#-------------------------------------------------------------------------------------------------------
# Hyper-parameters : TODO: modify this part of the script
#-------------------------------------------------------------------------------------------------------
### Data paths
if len(sys.argv) == 2: # Case with cross-validation enabled, the second argument is the subject index
  subjectIdx = sys.argv[1]
  dataPath = '/hdd/datasets/UniMiB-SHAR/windows_labels/CV_folds/' + str(subjectIdx) + '/'
  resultPath = '/home/prg/programming/python/UniMiB-SHAR/results/' # Path to save the evaluation metrics for each subject
else:
  dataPath = '/hdd/datasets/UniMiB-SHAR/windows_labels/20_train_10_test/'
  resultPath = ''

modelParamPath = '/home/prg/programming/python/UniMiB-SHAR/model' # Path to save the trained DNN model
dnnFeaturesPath = '/home/prg/programming/python/UniMiB-SHAR/dnn_features' # Path to save the DNN features computed with the trained model
...
selectedModel = hybrid # TODO: modify this value to change the model. Choices available: mlp, cnn, lstm, hybrid, ae 
...
#-------------------------------------------------------------------------------------------------------
# trainDnn: function to train a Deep Neural Network
#-------------------------------------------------------------------------------------------------------
def trainDnn(dataPath,
  ...
  ### load the UniMiB-SHAR data
  print('Loading the UniMiB-SHAR data from folder ' + dataPath)

  x_train = np.load(dataPath+'/training_data.npy')
  y_train = np.load(dataPath+'/training_labels.npy')
  x_test = np.load(dataPath+'/testing_data.npy')
  y_test = np.load(dataPath+'/testing_labels.npy')
  ...
  checkpoint = keras.callbacks.ModelCheckpoint('/tmp/checkpoint/checkpoint.hdf5',monitor='val_acc',mode='max',save_best_only=True,save_weights_only=False,period=1)
...
if __name__ == '__main__':
  ...
  print(dataPath)
  print('#############################################################')
  # Important NOTE: the script in its current form overwrites previously saved models in the case of the LOSO Cross-Validation
  # Change the name of the model accordingly if you need to save a model for each fold
